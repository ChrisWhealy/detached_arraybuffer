...
However, the declaration of the new `String` requires more memory than has currently been allocated, so, using the extra functions generated by `cargo`, shared memory is automatically and silently extended.
...
1. We code around the problem because we know that WebAssembly memory growth will break the corresponding JavaScript `ArrayBuffer`, or
1. We adjust the Rust coding so that memory growth is not needed.
...
To actually solve the problem, the Rust coding needs to avoid invoking any instructions that cause memory growth.
...
The Rust coding now performs exactly the same task without causing memory growth.
